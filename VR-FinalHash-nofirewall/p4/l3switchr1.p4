...
control MyIngress(inout headers hdr,
    ...
    action definetunnel(macAddr_t nextHopMac1, macAddr_t nextHopMac2) {
	    bit<2> hashResult;
	    bit<2> aux1 = 0;
	    bit<2> aux2 = 2;
            hash(hashResult, HashAlgorithm.crc16, aux1, {hdr.ipv4.srcAddr, hdr.ipv4.dstAddr}, aux2);
	    hdr.mslp[0].setValid();
	    hdr.mslp[1].setValid();
	    hdr.mslp[2].setValid();
	    ...
	    if (hashResult == 0) {
	       hdr.mslp[0].label = 0x2020;
               hdr.mslp[1].label = 0x3020;
	       standard_metadata.egress_spec = 2;
	       meta.nextHopMac = nextHopMac1;
	       //hdr.mslp.ttl = 255;
               hdr.ethernet.etherType = TYPE_MSLP;
            }if (hashResult == 1) {
	       hdr.mslp[0].label = 0x6020;
               hdr.mslp[1].label = 0x5020;
	       standard_metadata.egress_spec = 3;
	       meta.nextHopMac = nextHopMac2;
            }
            hdr.mslp[2].label = 0x4010;
	    hdr.ethernet.etherType = TYPE_MSLP;
	    //standard_metadata.egress_spec = egressPort;
	    //meta.nextHopMac = nextHopMac;
            //hdr.mslp.ttl = hdr.mslp.ttl - 1;
